var app = angular.module('myapp', ['angularUtils.directives.dirPagination']); //tao 1 module
app.controller('nhacungcapcontroller', function($scope, $http) { //tao 1 controller
    const id = localStorage.getItem('chosenId');
    
    $scope.openDetails = (id) => {
        // console.log(id)
        localStorage.setItem('chosenId', id);
        // window.open('/detail');
    }
    $scope.timkiem="";
    $scope.serial = 1;
    $scope.itemPerPage=5
    $scope.indexCount = function(newPageNumber){

        $scope.serial = newPageNumber * $scope.itemPerPage - ($scope.itemPerPage-1);
    }
    $scope.sort = function(keyname){
        $scope.sortBy = keyname; //the sort column name
        $scope.reverse =!$scope.reverse;// ASC/DESC sorting
    }
    $http({
        method: "GET",
        url: "http://localhost:8000/api/providers"
    }).then(function(response) {
        console.log(response.data);
        $scope.providers = response.data;
    });
    $http({
        method: "get",
        url: "http://localhost:8000/api/providers/" + id,
        
    }).then(function(response) {
        $scope.provider = response.data;
        console.log(response.data);
    }, err => console.log(err));
    $scope.showadd=function(id){
        if (id == 0) {
            if($scope.provider){
                $scope.provider.ten_ncc="";
            $scope.provider.dia_chi="";  
            $scope.provider.email="";
            $scope.provider.sdt=""; 
            }
            
        }
    };
    $scope.showmodal = function(id) {
        $scope.id = id;
        if (id == 0) {
            $scope.modalTitle = "Thêm nhà cung cấp mới";
            if($scope.provider){
                $scope.provider.ten_ncc="";
            $scope.provider.dia_chi="";  
            $scope.provider.email="";
            $scope.provider.sdt=""; 
            }
            
        } else {
            $scope.modalTitle = "Sửa thông tin";
            $http({
                method: "GET",
                url: "http://localhost:8000/api/providers/" + id
            }).then(function(response) {
                $scope.provider = response.data;
            });
        }
        $('#modelId').modal('show');
    }
    $scope.deleteClick = function(id) {
        var hoi = confirm("Ban co muon xoa that khong");
        if (hoi) {
            $http({
                method: "DELETE",
                url: "http://localhost:8000/api/providers/" + id
            }).then(function(response) {
                $scope.message = response.data;
                location.reload();
            });
        }
    }
    $scope.saveData = function() {
        if ($scope.id == 0) { //dang them moi sp
            // $scope.provider.anh=document.getElementById("img").innerHTML;
            $http({
                method: "POST",
                url: "http://localhost:8000/api/providers",
                data: $scope.provider,
                "content-Type": "application/json"
            }).then(function(response) {
                $scope.message = response.data;
                console.log(response.data);
                location.reload();

            });
        } else { //sua san pham
            // $scope.provider.anh=document.getElementById("img").innerHTML;
            $http({
                method: "PUT",
                url: "http://localhost:8000/api/providers/" + $scope.id,
                data: $scope.provider,
                "content-Type": "application/json"
            }).then(function(response) {
                $scope.message = response.data;
                console.log(response.data);
                location.reload();

            });
        }
    }
});